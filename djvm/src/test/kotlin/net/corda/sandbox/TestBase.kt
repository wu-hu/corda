...
open class TestBase {
    ...
    companion object {
        ...
        val FIXED_CLASSES_FOR_TEST = setOf(
                Regex("org/junit/.*"),
                Regex("org/assertj/.*")
        )
     ...
     */
    inline fun analyze(block: (ClassAndMemberVisitor.() -> Unit)) {
        val validator = RuleValidator(emptyList(), AnalysisConfiguration())
        block(validator)
    }
    ...
    fun sandbox(
            vararg options: Any,
            fixedClasses: Set<java.lang.Class<*>> = emptySet(),
        ...
        Thread {
            try {
                val fixedTestClasses = fixedClasses
                        .map { Regex(it.name.replace('.', '/').replace("$", "\\\$")) }
                        .toSet() + FIXED_CLASSES_FOR_TEST
                val analysisConfiguration = AnalysisConfiguration(
                        whitelist = Whitelist.TEST,
                        fixedClasses = Whitelist.FIXED_CLASSES + fixedTestClasses,
