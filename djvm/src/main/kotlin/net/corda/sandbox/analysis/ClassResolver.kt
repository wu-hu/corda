 ...
 * @property fixedClasses Classes and packages that should be left untouched.
 * @property sandboxPrefix The package name prefix to use for classes loaded into a sandbox.
 */
class ClassResolver(
        whitelist: Whitelist,
        private val fixedClasses: Whitelist,
        private val sandboxPrefix: String
) {

    /**
     * Classes and packages that are either fixed or already in the sandbox.
     */
    private val fixedClassesAndSandbox =
            whitelist + fixedClasses + Regex("^$sandboxPrefix.*$")
    ...
    fun reverse(resolvedClassName: String): String {
        if (fixedClasses.matches(resolvedClassName)) {
     ...
     */
    private fun resolveName(name: String) = if (name.isBlank() || isFixedClass(name)) {
        name
    } else {
        "$sandboxPrefix$name"
    }
    ...
    /**
     * Check if class name is whitelisted.
     */
    private fun isFixedClass(name: String): Boolean =
            fixedClassesAndSandbox.matches(name)
