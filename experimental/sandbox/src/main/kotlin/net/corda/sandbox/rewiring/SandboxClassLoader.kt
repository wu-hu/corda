...
class SandboxClassLoader(
     ...
     */
    private val fixedClasses = configuration.analysisConfiguration.fixedClasses
    ...
    fun loadClassAndBytes(source: ClassSource, context: AnalysisContext): LoadedClass {
        ...
        if (!configuration.analysisConfiguration.whitelist.matches(readClassName)) {
            if (configuration.analysisConfiguration.analyzeFixedClasses || !fixedClasses.matches(readClassName)) {
        ...
        if (fixedClasses.matches(qualifiedName)) {
            val fixedClass = LoadedClass(
                    supportingClassLoader.loadClass(name),
                    ByteCode(ByteArray(0), false)
            )
            loadedClasses[name] = fixedClass
            return fixedClass
        }
